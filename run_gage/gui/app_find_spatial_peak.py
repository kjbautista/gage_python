...
from run_gage.acquisition_workers import SpatialPeakWorker, LiveAcquisitionWorker, build_display_fft_mag
...
class FindSpatialPeakGui(QMainWindow):
    """PyQt front end for two-phase spatial peak finding (coarse raster + Nelder-Mead)."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle('Spatial Peak Finder')
    ...
    def _build_ui(self) -> None:
        ...
        display_layout.addWidget(self._wrap_group('Peak Map', self.beammap_canvas), 5)
    ...
    def _build_peak_search_group(self) -> QGroupBox:
        ...
        layout.addWidget(QLabel('Refinement tol (mm)'), 3, 0, 1, 2)
        layout.addWidget(self.refinement_tol_spin, 3, 2)
    ...
    def _start_peak_search_after_stop(self) -> None:
        ...
        self._start_worker_thread(worker)
        self._log_status(f'Starting peak search: {self.scan_positions.shape[0]} coarse positions.')
    ...
    def _log_search_summary(self) -> None:
        lines = [f'Total coarse scan positions: {self.scan_positions.shape[0]}']
        if self.groups_to_move[0]:
            group1_dims = np.sort(np.unique(self.scan_positions[:, 0]))
            lines.append(f'Group1: {group1_dims.min():.2f} to {group1_dims.max():.2f}')
    ...
    def _handle_search_point(self, position: object, value: float, step_index: int, total_positions: int) -> None:
        ...
        # Update position readout displays from probe positions during scan
        self.group1_pos_edit.setText(f'{pos[0]:.4f}')
        self.group2_pos_edit.setText(f'{pos[1]:.4f}')
        self.group3_pos_edit.setText(f'{pos[2]:.4f}')

        # Map 3D position to 2D heatmap indices
        ...
        # Only expand map if probing outside the initial coarse grid (refinement phase)
        if row_index < self.live_beammap.shape[0] and col_index < self.live_beammap.shape[1]:
    ...
    def _handle_phase_changed(self, phase: str) -> None:
        if phase == 'coarse':
            self.phase_label.setText('Coarse raster scan...')
        elif phase == 'refine':
            self.phase_label.setText('Nelder-Mead refinement...')
        else:
